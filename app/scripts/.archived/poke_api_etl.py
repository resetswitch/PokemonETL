...
def apiendpoint0_to_db(generation: int, data: dict) -> int|None:
    ...
    if species is not None:
        ...
        try:
            ...
            logger.error(f"Unexpected error: {exc}. Generation {generation}", exc_info=True)
            return 0
            print(f"Unexpected error: Generation {generation}")
    return None

def apiendpoint0_transact(generations:list[int]) -> None:
    """
...
def apiendpoint1_get_missing_poke_ids() -> list[int]:
    """
...
def apiendpoint1_to_db(poke_id: int,data: dict) -> int:
    """
    ...
    try:
        ...
        logger.error(f"Issues upserting poke_id:{poke_id}.Error Message: {exc}")
        return 0

def apiendpoint1_transact(poke_ids: list[int]) -> None:
    """
    ...
    with Progress(
        ...
        task = prog.add_task("Rqsting APIendpoint1", total=len(poke_ids))
        ...
        for poke_id in poke_ids:
            ...
            try:
                ...
                data = get_json(url)
                apiendpoint1_to_db(poke_id, data)
            except Exception as exc:
                logger.error("All retries exhausted: %s", exc)
...
def apiendpoint2_get_missing_poke_ids() -> list[int]:
    """
    generates the missing poke_ids based on values from the pokemon_species vs
    the pokemon_description1 table. This descerns which poke_ids to call from
...
def apiendpoint2_to_db(poke_id: int,data: dict) -> int:
    """
...
def apiendpoint2_transact(poke_ids: list[int]) -> None:
    """
    Transforms data from "https://pokeapi.co/api/v2/pokemon-species/{poke_id}/" endpoint
    and uploads to several tables in pokemon database:
    - pokemon_description2: (poke_id, genus, shape, habitat, color,
    ...
    with Progress(
        ...
        task = prog.add_task("Rqsting APIendpoint2", total=len(poke_ids))
        ...
        for poke_id in poke_ids:
            ...
            try:
                ...
                data = get_json(url)
                apiendpoint2_to_db(poke_id, data)
            except Exception as exc:
                logger.error("All retries exhausted: %s", exc)
...
if __name__ == "__main__":
    ...
    apiendpoint0_transact(generations)

    poke_ids_apiep1 = apiendpoint1_get_missing_poke_ids()
    apiendpoint1_transact(poke_ids_apiep1)

    poke_ids_apiep2 = apiendpoint2_get_missing_poke_ids()
    apiendpoint2_transact(poke_ids_apiep2)
